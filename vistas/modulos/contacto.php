
<div id="services" class="section lb">
    <div class="container-fluid">
		<div class="container">
			<div class="section-title text-center">
				<h3>Contáctanos</h3>
				<p class="lead">Te agradecemos tu interés en contactarnos. Nos encantará poder contestar cualquier pregunta que puedas tener acerca de Cucas Kain, sobre nuestros servicios y productos.</p>
			</div><!-- end title -->
        </div><!-- end row -->
    </div><!-- end container -->
</div><!-- end section -->
<br><br>
<div class="container">
<div class="row">
    <div class="col-md-8 offset-md-2">
        <div class="contact_form" style="border-radius: 2.5rem;background-color:#ff5e00;">
            <div id="message"></div>
                <form id="contactform" action="contact" name="contactform" method="post">
                    <fieldset class="row">
                        <div class="col-lg-12 col-md-12 col-sm-6">
                            <input type="text" name="first_name" id="first_name" class="form-control" placeholder="Nombre">
                        </div>
                        <div class="col-lg-12 col-md-12 col-sm-6">
                            <input type="text" name="last_name" id="last_name" class="form-control" placeholder="Apellido">
                        </div>
                        <div class="col-lg-12 col-md-12 col-sm-6">
                            <input type="email" name="email" id="email" class="form-control" placeholder="Correo">
                        </div>
                        <div class="col-lg-12 col-md-12 col-sm-6">
                            <input type="text" name="phone" id="phone" class="form-control" placeholder="Teléfono">
                        </div>
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <textarea class="form-control" name="comments" id="comments" rows="6" placeholder="¿Qué necesitas de nosotros?"></textarea>
                        </div>
                        <div class="text-center cont-btn">
                            <button type="submit" value="SEND" id="submit" class="bttn-new"><span>Enviar</span></button>
                        </div>
                    </fieldset>
                </form>
            </div>
        </div><!-- end col -->
    </div><!-- end row -->

    <div class="row">
		<div class="offset-md-1 col-sm-10 col-md-10 offset-sm-1 pd-add">
			<div class="address-item">
				<div class="address-icon">
                    <i class="fas fa-map-marked-alt"></i>
				</div>
				<h3>Ubicación</h3>
				<p>Monterrey, Nuevo León</p>
			</div>
			<div class="address-item">
				<div class="address-icon">
                    <i class="fas fa-mail-bulk"></i>
				</div>					
                <h3>Correo</h3>
				<p>cucaskain@contacto.com</p>
			</div>
			<div class="address-item">
				<div class="address-icon">
                    <i class="fas fa-phone"></i>
				</div>
				<h3>Télefono</h3>
				<p>+9999999999</p>
			</div>
		</div>
	</div><!-- end row -->
</div><!-- end row -->
</div><!-- end container -->